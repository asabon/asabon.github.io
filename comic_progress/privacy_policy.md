...
ユーザーは、デバイスの設定から広告IDをリセットしたり、関心に基づく広告のオプトアウトを行うことができます。
具体的な方法については、以下のリンクを参照してください。
...
このアプリは、使用者のデータ保護、復元を目的として、 Google の Android Auto Backup 機能を使用しています。
このバックアップ機能により、アプリの設定やデータが自動的に Google のクラウドストレージに保存され、新しい端末にデータ移行する際に自動的に復元されます。

## バックアップされるデータ

アプリ上でユーザーによって入力されたデータ。

## データ保存の仕組み

バックアップデータは、ユーザーの Google アカウントに関連付けられ、Google の安全なクラウドストレージに保存されます。
バックアップおよび復元のプロセスは、ユーザー操作なしに自動で行われます。
